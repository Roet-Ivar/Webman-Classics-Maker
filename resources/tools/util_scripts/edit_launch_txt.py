        ...
        try:
            ...
            if "/PSPISO/" in path:
                web_command_string = (
                    "/mount_ps3"
                    + path
                    + ";/wait.ps3?8;/browser.ps3$focus_segment_index xmb_app3 0;/wait.ps3?1;/browser.ps3$exec_push;/wait.ps3?1;/browser.ps3$focus_index 0 4;/wait.ps3?1;/browser.ps3$exec_push;/wait.ps3?1;/browser.ps3$exec_push;/wait.ps3?1;/browser.ps3$exec_push"
                )
            ...
            # check if the user has added a custom webcommand in the config file
                ...
                if len(cfg_webcommand) > len("[filepath_var]"):
                    if "[filepath_var]" in cfg_webcommand:
                        web_command_string = cfg_webcommand.replace(
                            "[filepath_var]", full_path
                        )
                        web_command_string = web_command_string.replace("//", "/")
                    else:
                        print(
                            """Error: make sure the string [filepath_var] (including brackets) is present in webcommand of settings.cfg"""
                        )
            ...
            web_url_string = (
                "GET " + urllib.parse.quote(web_command_string) + " HTTP/1.0"
            )

            if not os.path.exists(
                os.path.join(AppPaths.game_work_dir, "pkg", "USRDIR")
            ):
                os.makedirs(os.path.join(AppPaths.game_work_dir, "pkg", "USRDIR"))

            launch_txt = open(
                os.path.join(AppPaths.game_work_dir, "pkg", "USRDIR", "launch.txt"),
                "wb",
            )
            launch_txt_byteArray = bytearray(
                web_command_string, "utf8"
            ) + os.linesep.encode("utf-8")
            print("Launch command is:", launch_txt_byteArray)
            launch_txt.write(launch_txt_byteArray)

            url_txt = open(
                os.path.join(AppPaths.game_work_dir, "pkg", "USRDIR", "url.txt"), "wb"
            )
            url_txt_byteArray = bytearray(web_url_string, "utf8") + os.linesep.encode(
                "utf-8"
            )
