...
games_string = ''
print(os.path.dirname('./games_metadata/GamesIndex.txt'))
try:
    with open('./games_metadata/GameIndex.txt', 'r') as f:
        # print(f.read())
        title_id    = ''
        title       = ''
        region      = ''
        ...
        f.close()
        with open('./games_metadata/GamesIndex.json', "w") as newFile:
            json_text = json.dumps(j_body, indent=4, separators=(",", ":"), ensure_ascii=False).encode('utf8')
            newFile.write(json_text)
        # with open('./games_metadata/GamesIndex_str.txt', "w") as newFile:
